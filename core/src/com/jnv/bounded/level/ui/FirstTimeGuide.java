...
public class FirstTimeGuide {
	...
	private int level;
	private List<Image> levelHelperMasks, levelHelpers;
	...
	public FirstTimeGuide(final int level, GameStateManager gsm, final LevelState levelState) {
		...
		if (TutorialDialogCoords.allCoords[level - 1].length > 0) levelState.setDrawable(false);
		...
		for (int i = 0; i < TutorialDialogCoords.allCoords[level - 1].length; i++) {
	...
	public void addToStage() {
		if (TutorialDialogCoords.allCoords[level - 1].length > 0) {
	...
	private void addListener(final int level, final int index, final Runnable action) {
		triggerMasks.get(index).addListener(new InputListener(triggerMasks.get(index)) {
			...
			public void doAction() {
				...
				if (index < TutorialDialogCoords.allCoords[level - 1].length - 1) {
